...
contract SplitterStrategy is IStrategyV2, Initializable, ControllableInit, SplitterStorage {
  ...
  function controller() public view override(IStrategyV2, ControllableInit) returns (address) {
    return Storage(_storage()).controller();
  }
  function governance() public view override(IStrategyV2,GovernableInit) returns (address) {
    return Storage(_storage()).governance();
  }
  function underlying() public view override(IStrategyV2,SplitterStorage) returns (address) {
    return getAddress(_UNDERLYING_SLOT);
  }
  function vault() public view override(IStrategyV2,SplitterStorage) returns (address) {
  ...
  */
  ...
  function unsalvagableTokens(address token) public view virtual override returns (bool) {
  ...
  */
  function investAllUnderlying() internal {
    ...
    for (uint256 i = 0; i < activeStrategies.length; i++) {
      ...
      uint256 toInvest = MathUpgradeable.min(computedRatio, IERC20(underlying()).balanceOf(address(this)));
      if (toInvest > 0) {
  ...
  */
  function withdrawToVault(uint256 correspondingShares, uint256 totalShares) external restricted override {
  ...
  */
  function doHardWork() public restricted override {
    investAllUnderlying();
    for (uint256 i = 0; i < activeStrategies.length; i++) {
  ...
  */
  function whitelistStrategy(address _strategy) public onlyGovernance {
    ...
    IERC20(underlying()).safeApprove(_strategy, type(uint256).max);
    finalizeStrategyWhitelist();
  }

  // should only be called by controller
  function salvage(address destination, address token, uint256 amount) external restricted override{
