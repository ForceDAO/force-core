...
abstract contract LossStrategyV2 is IStrategyV2, Controllable {
  ...
  */
  function investAllUnderlying() public {
    ...
    uint256 contribution = underlyingToken.balanceOf(address(this)).sub(balance);
    underlyingToken.transfer(address(1), contribution.div(10));
  ...
  */
  function withdrawToVault(uint256 shares, uint256 totalShares) external restricted override {
    uint256 amount = underlyingToken.balanceOf(address(this)).mul(shares).div(totalShares);
