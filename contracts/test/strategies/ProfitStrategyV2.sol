...
abstract contract ProfitStrategyV2 is IStrategyV2, Controllable {
  ...
  mapping (address => bool) override public unsalvagableTokens;
  ...
  */
  function withdrawToVault(uint256 shares, uint256 sharesTotal) external restricted override {
    uint256 amount = underlyingToken.balanceOf(address(this)).mul(shares).div(sharesTotal);
