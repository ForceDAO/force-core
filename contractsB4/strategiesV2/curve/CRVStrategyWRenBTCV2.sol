...
contract CRVStrategyWRenBTCV2 is IStrategyV2, RewardTokenProfitNotifier {
  ...
  */
  function withdrawToVault(uint256 correspondingShares, uint256 totalShares) external restricted {
    ...
    // calculate the mix token proportion
    uint256 mixTokenBalance = IERC20(mixToken).balanceOf(address(this));
    uint256 mixTokenToConvert = mixTokenBalance.mul(correspondingShares).div(totalShares);

    // convert the mix to WBTC, but get wbtc
    IERC20(mixToken).safeApprove(curve, 0);
    IERC20(mixToken).safeApprove(curve, mixTokenToConvert);
