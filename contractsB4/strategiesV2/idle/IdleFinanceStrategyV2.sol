...
contract IdleFinanceStrategyV2 is IStrategyV2, RewardTokenProfitNotifier {
  ...
  */
  function investedUnderlyingBalance() public view returns (uint256) {
    ...
    return IERC20(underlying).balanceOf(address(this)).add(
      IERC20(idleUnderlying).balanceOf(address(this)).mul(virtualPrice).div(1e18)
    );
  }
